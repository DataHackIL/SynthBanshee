...
| **M3c** | P0 | ✅ Done | Restores absolute clip loudness (Whisper / UTMOS) without re-collapsing M3a contrast (#78) | `peak_normalize_to_target()` helper in `synthbanshee/augment/preprocessing.py` (single global gain); `PreprocessingConfig.target_peak_dbfs: float = −2.0` (range [−12, −1]); `preprocess()` runs target-peak step before the −1.0 dBFS safety limiter; `cli.py` Stage 3b post-augment normalize uses the same helper + same config so Tier A and Tier B/C exit at the same absolute peak; integration regression test asserting peak in [−3, −1] dBFS; update `spec.md §3` and §4.7 | Small | Yes — spec.md §3, §4.7 |
...
2. **Two-stage clip-level loudness normalization (#78, supersedes the original M3b limiter-only behaviour):**

   - **5a. Peak-normalize to target via single global gain.** A single multiplicative factor is applied to the *entire* mixed scene so its peak lands at `PreprocessingConfig.target_peak_dbfs` (default −2.0 dBFS, range [−12.0, −1.0]). Single-gain normalization is the key invariant: it preserves *every* per-turn RMS *ratio* exactly, so the within-scene loudness trajectory created by step 1 survives — only the absolute level shifts. This is *not* the old per-turn divide-by-max normalizer that M3b removed; that one collapsed contrast because it scaled each segment to its own peak. The new step normalizes after mixing, once, globally.
   - **5b. Safety limiter at ≤ −1.0 dBFS.** Defence-in-depth only: attenuates over-range samples without scaling up. For all in-spec target values it is a no-op.

   **History.** PR #25 (M3a) added per-turn RMS targeting; PR #27 (M3b) replaced the legacy peak normalizer with a peak-only limiter to stop it from erasing M3a's per-turn contrast. M3b succeeded at preserving contrast but left the absolute clip peak at whatever the M3a-targeted RMS produced — typically ~−6 dBFS for Tier A. M17 Phase A discovered (issue #78) that this confused Whisper (WER 0.04 → 0.28, length-ratio 0.76) and UTMOS (~0.9 MOS swing on limiting characteristics alone). Step 5a above restores a useful absolute loudness floor *without* reverting M3b's contrast-preservation property, because a single global gain is mathematically incapable of compressing or expanding within-clip RMS ratios.

   **Tier B/C augmentation re-applies step 5a** after room-IR + noise mixing, since that pipeline reshapes the peak. Both tiers exit at `target_peak_dbfs`, so M17 evaluation sees a consistent loudness regime across the dataset.
