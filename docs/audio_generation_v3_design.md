...
| **#78** | P1 | ✅ Done | Defines a clip-level loudness contract (target peak + safety ceiling) and a metadata trail so future loudness regressions are diagnosable from `{clip_id}.json` alone | `peak_normalize_to_target()` helper in `synthbanshee/augment/preprocessing.py` (single global gain — preserves M3a contrast); `PreprocessingConfig.target_peak_dbfs: float = −2.0` (range `[−12.0, −1.5]`, 0.5 dB headroom over the safety limiter); `preprocess()` runs the target-peak step before the −1.0 dBFS safety limiter; `cli.py` Stage 3b post-augment normalize uses the same helper + same config so Tier A and Tier B/C exit at the same absolute peak (Tier B/C target moves from −1.0 → −2.0 dBFS as a side effect — gives 1 dB inter-sample-peak headroom for room-IR/noise mixing); `GenerationMetadata` adds `loudness_target_peak_dbfs` and bumps `normalization_strategy` from `per_turn_rms_v1` → `per_turn_rms_v2_target_peak`; `PreprocessingResult` adds `target_peak_dbfs` field; step-name strings flattened to literal tokens (`peak_normalize`, `peak_limit`, `silence_pad`); integration regression tests with realistic-crest fixture asserting peak deviation < 0.1 dB and per-turn RMS contrast within 0.2 dB; spec updated. **Empirical note (lever probe, 2026-05-05):** peak normalization at this level does not change Whisper WER — Whisper's log-mel extractor internally normalizes — so this is a contract / metadata fix, not an ASR fix.  The M17 ASR regression has a separate root cause tracked in the prosody-bisect follow-up. | Small | Yes — spec.md §3, §3.1, §4.7; AGENTS.md |
...
| **M5** | P1 | ✅ Done | Protects Tier A phonetic quality from over-denoising | `PreprocessingConfig` Pydantic `BaseModel` with `wiener_denoise` flag; update `preprocess()` to accept it (default preserves current Wiener behavior); scene YAMLs gain optional `preprocessing` block; Wiener step skipped when `wiener_denoise=False`; unit tests verify Wiener skip | Small | No |
| **M6** | P1 | ✅ Done | Replaces mechanical silence gaps with psychologically-motivated turn latencies | `TurnGapController` in `synthbanshee/tts/gap_controller.py`; project-specific gap tables (§4.5); wire into `TTSRenderer.render_scene()` replacing `turn.pause_before_s`; NEG/NEU confusor gap ranges; unit tests per context type and project | Small | No |
| **M7** | P1 | ✅ Done | Eliminates speaker-state reset at turn boundaries | `SpeakerState` with `update()` / `to_metadata_dict()`; intensity-drift rules per role; wire into `TTSRenderer.render_scene()` — one `SpeakerState` per speaker; state offsets applied to render params; state written to per-turn generation metadata | Medium | No |
| **M2b** | P1 | ✅ Done | Burst-pause microstructure within high-intensity turns | `PhraseHint` / `PhraseProsody` dataclasses in `ssml_types.py`; `DialogueTurn.phrase_hints`; char-offset resolver (maps `text_original` offsets → `text_spoken` after niqqud); `SSMLBuilder` extension for nested `<prosody>` + `<break>`; Jinja2 template update requesting `phrase_hints` for I3–I5 turns; deterministic sentence-final imperative heuristics | Medium | No |
| **M8a** | P1 | ✅ Done | Adds interruption behavior to audio assembly | `MixMode` enum (`SEQUENTIAL`, `OVERLAP`, `BARGE_IN`); extend `SceneMixer.mix_sequential()` to handle overlap/barge-in segment mixing; `TurnGapController.gap_seconds()` extended to return `(gap_s, MixMode)`; unit tests for overlapped segment merge | Medium | No |
| **M8b** | P1 | ✅ Done | Precise event timestamps for overlapping turns; NEG/NEU confusor overlap | `MixedScene` gains `script_onset_s`, `rendered_onset_s`, `audible_onset_s` per turn; label generator uses `audible_onset_s`/`audible_end_s` exclusively; truncated turns get `truncated: true`; NEG/NEU overlap rates; integration tests | Medium | No |
| **M9a** | P2 | ✅ Done | Second TTS backend; eliminates single-backend fingerprint risk | `provider.py` — `ProviderCapabilities` dataclass + `TTSProvider` Protocol; `google_provider.py` — Google Cloud TTS Chirp 3 HD he-IL; `SSMLBuilder` `supports_style_tags` flag (omits `<mstts:express-as>` for Google); `TTSRenderer` multi-provider dispatch via `providers=` dict + `_legacy_mode` for backward compat; speaker YAMLs `AGG_M_30-45_002` / `VIC_F_25-40_003` (Google voices) | Medium | No |
...
2. **Two-stage clip-level loudness normalization (#78, extends the original M3b limiter-only behaviour with an explicit target floor):**

   - **5a. Peak-normalize to target via single global gain.** A single multiplicative factor is applied to the *entire* mixed scene so its peak lands at `PreprocessingConfig.target_peak_dbfs` (default −2.0 dBFS, range `[−12.0, −1.5]`). Single-gain normalization is the key invariant: it preserves *every* per-turn RMS *ratio* exactly, so the within-scene loudness trajectory created by step 1 survives — only the absolute level shifts. This is *not* the old per-turn divide-by-max normalizer that M3b removed; that one collapsed contrast because it scaled each segment to its own peak. The new step normalizes after mixing, once, globally.
   - **5b. Safety limiter at ≤ −1.0 dBFS.** Defence-in-depth only: attenuates over-range samples without scaling up. The 0.5 dB margin between the target upper bound (−1.5) and the limiter ceiling (−1.0) guarantees this stage is a no-op for all in-spec target values.

   **History.** PR #25 (M3a) added per-turn RMS targeting; PR #27 (M3b) replaced the legacy peak normalizer with a peak-only limiter to stop it from erasing M3a's per-turn contrast. M3b succeeded at preserving contrast but left the absolute clip peak unspecified — it floated wherever the M3a-targeted RMS produced (typically ~−6 dBFS for Tier A). The M17 Phase A spike noted that quieter clips also showed elevated Whisper WER and shifted UTMOS; issue #78 was filed under the hypothesis that loudness drove the ASR regression. **A lever probe over the same source audio at peaks ranging from −6 to +7 dBFS (and RMS from −27 to −14 dBFS) showed Whisper produces byte-identical hypotheses at every peak level in the spec range** — Whisper's log-mel extractor internally normalizes, so absolute loudness is invisible to it. Step 5a is therefore *not* an ASR fix; it is a **loudness-contract fix**: the spec previously had only an upper bound (`peak ≤ −1.0 dBFS`), no target floor, so two clips could legitimately sit 6 dB apart and both be in-spec. With step 5a in place, target peak is configured policy and `loudness_target_peak_dbfs` in `GenerationMetadata` carries the value used so future loudness drift is diagnosable from clip metadata alone.

   **Tier B/C augmentation re-applies step 5a** after room-IR + noise mixing, since that pipeline reshapes the peak. Both tiers exit at `target_peak_dbfs`, so the dataset has a consistent loudness regime across tiers (it previously did not — Tier A had no target, Tier B/C hardcoded −1.0).

   **The Whisper WER regression itself remains open** — it is real but loudness-independent. Lever-probe data and root-cause investigation tracked separately (prosody-bisect follow-up issue): most likely candidates are the M15 prosody tuning (#51) and the inter-word `<break>` tags from #70.
...
4. **Spec text:** `docs/spec.md` §3 row "Amplitude normalization" and §3.1 step 5 are kept in sync with the two-stage policy above.
