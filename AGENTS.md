...
- **M3a — per-turn RMS gain:** `StyleEntry.rms_target_dbfs` → module-level `_apply_rms_gain()` in `synthbanshee/tts/mixer.py`. Segment-passing API is the `Segment` dataclass in `mixer.py` (fields: `wav_bytes`, `amount_s`, `speaker_id`, `rms_target_dbfs`, `mix_mode`, `intensity`); it replaced the original M3a 4-tuple as M8a added `mix_mode` and #65 added `intensity` for the I4/I5 Lombard high-shelf — named fields keep call sites from transposing positional args.
