...
class GoogleProvider:
    ...
    The client is lazily created on the first ``synthesize()`` call and
    reused for all subsequent calls (avoids repeated gRPC channel setup).
    ...
    def synthesize(self, ssml: str) -> bytes:
        ...
        try:
            response = client.synthesize_speech(
                input={"ssml": ssml},
                voice={"language_code": lang_code, "name": voice_name},
                audio_config={
                    "audio_encoding": "LINEAR16",
                    "sample_rate_hertz": _OUTPUT_SAMPLE_RATE_HZ,
                },
            )
...
def _extract_voice_name(ssml: str) -> str:
    ...
    Uses regex rather than XML parsing because the SSML is always
    machine-generated by ``SSMLBuilder`` — attribute ordering and
    quoting style are deterministic and well-known.
    """
    import re
